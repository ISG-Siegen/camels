"""
Original code from Lenskit. Extended for CaMeLS.
"""
...
class BookCrossing:
    """
    Code to read the BookCrossing data set.

    Parameters:
        path(str or pathlib.Path): Path to the directory containing the data set.
    """

    def __init__(self, path='data/book-crossing'):
        self.path = Path(path)
    ...
    def ratings(self):
        ...
        # Change ISBN to IDs
        item_ids = {key: value for value, key in enumerate(ratings['ISBN'].unique())}
        ratings["ISBN"].update(ratings["ISBN"].map(item_ids))
...
class Amazon:
    """
    Code to read the Amazon data sets.
    ...
    @cached
    def ratings(self):
        file_name = Path(str(os.listdir(self.path)[0]))

        fn = self.path / file_name
        ratings = pd.read_json(fn, lines=True, dtype={
            'reviewerID': str,
            'asin': str,
            'overall': np.int32,
        ...
        # Change reviewerID to user IDs
        user_ids = {key: value for value, key in enumerate(ratings['reviewerID'].unique())}
        ratings["reviewerID"].update(ratings["reviewerID"].map(user_ids))

        # Change asin to item IDs
        item_ids = {key: value for value, key in enumerate(ratings['asin'].unique())}
        ratings["asin"].update(ratings["asin"].map(item_ids))

        ratings.rename(
            columns={'reviewerID': 'user', 'asin': 'item', 'overall': 'rating', 'unixReviewTime': 'timestamp'},
            inplace=True)
        _log.debug('loaded %s, takes %d bytes', fn, ratings.memory_usage().sum())
        return ratings
...
class EachMovie:
    """
    Code to read the EachMovie data set.

    Parameters:
        path(str or pathlib.Path): Path to the directory containing the data set.
    """

    def __init__(self, path='data/eachmovie_triple'):
        self.path = Path(path)
    ...
    def ratings(self):
        ...
        fn = self.path / file_name
        ratings = pd.read_csv(fn, delim_whitespace=True, header=None,
...
class Jester:
    """
    Code to read the Jester data sets.
    ...
    def ratings(self):
        ...
        fn = self.path / file_name
        ratings = pd.read_excel(fn, sheet_name=0, header=None).loc[:, 1:]
        ratings["user"] = [i for i in range(len(ratings))]
        ratings = ratings.melt(id_vars="user", var_name="item", value_name="rating")
        ratings = ratings[ratings["rating"] != 99]
        ratings.dropna(subset=["rating"], inplace=True)
        ratings = ratings.loc[(-10 <= ratings["rating"]) & (ratings["rating"] <= 10)]
